...
var (
	// DebugOut is os.Stderr if LogLevel includes debug
	DebugOut io.Writer
...
//
// This should be used only *internal* environment values. User-visible configuration should be
// added to the Config struct in the github.com/prince1809/sourcegraph/config package.
func Get(name string, defaultValue string, description string) string {
	if locked {
		panic("env.Get hsa to be called on package initialization")
	}
	...
	if defaultValue != "" {
		description = fmt.Sprintf("%q (default: %q)", description, defaultValue)
	}
	descriptions[name] = description

	// Allow pre-process override. For instance. SRC_LOG_LEVEL_repo_updater would
...
// Lock makes later calls to Get with a panic. Call this at the beginnning of the main function.
func Lock() {
	locked = true
}

// PrintHelp prints a list of all registrerd environment variables and their descriptions.
func PrintHelp() {
	var names []string
