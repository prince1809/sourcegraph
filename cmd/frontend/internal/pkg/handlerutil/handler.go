...
import (
	...
	"github.com/prince1809/sourcegraph/pkg/errcode"
...
// returns an error. If the error is nil, this wrapper is no-op. if
...
// (for example, call out into an external code), then it muse use recover
...
type HandlerWithErrorReturn struct {
	...
	Error   func(http.ResponseWriter, *http.Request, int, error) // called to send an error response
}
...
func (h HandlerWithErrorReturn) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		if e := recover(); e != nil {
			log15.Error("panic in HandleWithErrorReturn.Handler", "error", e)
...
// httpErrCode maps an error to a status code. If the client cancelled the
