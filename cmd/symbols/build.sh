    ...
    if ! command -v pkg-config >/dev/null 2>&1 || ! command -v pkg-config --cflags sqlite3 libpcre >/dev/null 2>&1; then
        ...
        case "$OSTYPE" in
            ...
            linux*)
                ...
                ;;
            *)
                echo "See the local development documentation: https://github.com/prince1809/sourcegraph/blob/master/doc/dev/local_development.md#step-2-install-dependencies"
                ;;
        esac
        exit 1
    fi
...
function  buildExecutable() {
    symbolsPackage="github.com/prince1809/sourcegraph/cmd/symbols"
